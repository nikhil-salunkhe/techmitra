...
async function sendBusinessEnquiryEmail(enquiry) {
  const mailOptions = {
    from: RESEND_FROM,
    to: enquiry.email,
    ...
    html: `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Business Enquiry Received - TechMitra</title></head>
<body style="margin:0; padding:0; background-color:#e8edf3; font-family:'Segoe UI', Arial, Helvetica, sans-serif;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#e8edf3; padding:20px 0;">
    <tr><td align="center" style="padding:20px 10px;">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="width:600px; max-width:100%; margin:0 auto; background-color:#ffffff; border-radius:16px; overflow:hidden; box-shadow:0 6px 30px rgba(0,0,0,0.08);">
        <tr>
          <td style="background:linear-gradient(135deg, #059669 0%, #0d9488 100%); padding:30px 20px; text-align:center;">
            <h1 style="color:#ffffff; font-size:24px; font-weight:700; margin:0 0 4px 0;">🏢 Thank You, ${enquiry.contactPerson}!</h1>
            <p style="color:#d1fae5; font-size:14px; margin:0;">Your enquiry has been received. Our team will contact you within 24 hours.</p>
          </td>
        </tr>
        <tr><td style="padding:30px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:10px; border-collapse:collapse;">
            <tr><td><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;">
              ...
              ${detailRow('Description', enquiry.description || 'Not provided')}
              ${detailRow('Status', '<span style="display:inline-block; background:#fef3c7; color:#b45309; padding:2px 10px; border-radius:20px; font-size:12px; font-weight:700;">🆕 NEW</span>', true)}
            </table></td></tr>
          </table>
          <p style="color:#6b7280; font-size:14px; margin:20px 0 0 0; line-height:1.6;">
            Our experts will review your requirements and reach out with a customized solution and quotation.
            For urgent queries, WhatsApp us at <a href="https://wa.me/919764149564" style="color:#059669; text-decoration:none; font-weight:600;">+91 97641 49564</a>.
          </p>
        </td></tr>
        <tr><td style="background:#f1f5f9; padding:24px; text-align:center; font-size:13px; color:#64748b; line-height:1.8;">
          <p style="margin:0; font-weight:600; color:#1e293b;">TechMitra Training Solutions | Building Future Tech Leaders</p>
          <p style="margin:4px 0;">📍 India (Online) | 📧 techmitrofficial@gmail.com | 📞 +91 97641 49564</p>
        </td></tr>
      </table>
...
async function sendBusinessEnquiryNotificationToAdmin(enquiry) {
  const adminUrl = `${FRONTEND_URL}/admin`;
  const mailOptions = {
    from: RESEND_FROM,
    to: ADMIN_EMAIL,
    ...
    html: `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>New Business Enquiry Notification - TechMitra</title></head>
<body style="margin:0; padding:0; background-color:#e8edf3; font-family:'Segoe UI', Arial, Helvetica, sans-serif;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#e8edf3; padding:20px 0;">
    <tr><td align="center" style="padding:20px 10px;">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="width:600px; max-width:100%; margin:0 auto; background-color:#ffffff; border-radius:16px; overflow:hidden; box-shadow:0 6px 30px rgba(0,0,0,0.08);">
        <tr>
          <td style="background:linear-gradient(135deg, #059669 0%, #2563eb 100%); padding:30px 20px; text-align:center;">
            <h1 style="color:#ffffff; font-size:24px; font-weight:700; margin:0 0 4px 0;">💼 New Business Enquiry!</h1>
            <p style="color:#d1fae5; font-size:14px; margin:0;">A new business client is interested in our services</p>
          </td>
        </tr>
        <tr><td style="padding:30px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:16px;">
            <tr><td align="center" style="padding:8px 0;">
              <span style="display:inline-block; background:#059669; color:#ffffff; padding:6px 16px; border-radius:20px; font-size:14px; font-weight:700;">${enquiry.id}</span>
              <span style="display:inline-block; background:#dbeafe; color:#1d4ed8; padding:4px 12px; border-radius:12px; font-size:12px; font-weight:500; margin-left:4px;">${enquiry.businessType}</span>
              <span style="display:inline-block; background:#ccfbf1; color:#0f766e; padding:4px 12px; border-radius:12px; font-size:12px; font-weight:500; margin-left:4px;">${enquiry.serviceType}</span>
            </td></tr>
          </table>
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:10px; border-collapse:collapse;">
            <tr><td><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;">
              ...
              ${detailRow('Timeline', enquiry.timeline || 'Flexible')}
              ${detailRow('Description', enquiry.description || 'Not provided')}
              ${detailRow('Submitted At', new Date(enquiry.createdAt).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata', dateStyle: 'full', timeStyle: 'short' }))}
              ${detailRow('Status', '<span style="display:inline-block; background:#fef3c7; color:#b45309; padding:2px 10px; border-radius:20px; font-size:12px; font-weight:700;">🆕 NEW</span>', true)}
            </table></td></tr>
          </table>
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-top:24px; background:#eff6ff; border:2px solid #bfdbfe; border-radius:10px;">
            <tr><td align="center" style="padding:20px;">
              <p style="margin:0 0 8px 0; color:#1e3a5f; font-weight:700; font-size:14px;">🔐 Manage This Business Enquiry</p>
              <a href="${adminUrl}" style="display:inline-block; background:#059669; color:#ffffff; text-decoration:none; padding:12px 32px; border-radius:8px; font-weight:700; font-size:14px;">💼 Open Admin Panel</a>
            </td></tr>
          </table>
        </td></tr>
        <tr><td style="background:#f1f5f9; padding:24px; text-align:center; font-size:13px; color:#64748b; line-height:1.8;">
  ...
  try {
    ...
    console.log(`⚠️ Business enquiry admin notification failed: ${error.message}`);
    return false;
  }
}
...
// ============ BUSINESS ENQUIRY ROUTES ============

// POST /api/business-enquiry - Create new business enquiry
app.post('/api/business-enquiry', async (req, res) => {
  try {
    const {
      businessName,
      contactPerson,
    ...
    // Validation
    if (!businessName || !contactPerson || !email || !phone || !businessType || !serviceType) {
      return res.status(400).json({
        success: false,
        message: 'Please fill all required fields (business name, contact person, email, phone, business type and service type).',
      });
    }
    ...
    const seq = await getNextSequence('businessEnquiryId');
    const enquiry = new BusinessEnquiry({
      id: `BUS-${seq}`,
      businessName,
      contactPerson,
      email,
    ...
    // Send emails in background (non-blocking)
    sendBusinessEnquiryEmail(enquiry).catch(error => {
      console.log(`⚠️ Background client email failed: ${error.message}`);
    });
    sendBusinessEnquiryNotificationToAdmin(enquiry).catch(error => {
      console.log(`⚠️ Background admin notification failed: ${error.message}`);
    });
...
// GET /api/business-enquiries - Get all business enquiries (for admin)
app.get('/api/business-enquiries', async (req, res) => {
  try {
    const enquiries = await BusinessEnquiry.find().sort({ createdAt: -1 });
    res.json({ success: true, enquiries });
  } catch (error) {
    console.error('Fetch business enquiries error:', error);
    res.status(500).json({ success: false, message: 'Server error' });
  }
});

// GET /api/business-enquiries/:id - Get single business enquiry
app.get('/api/business-enquiries/:id', async (req, res) => {
  try {
    const enquiry = await BusinessEnquiry.findOne({ id: req.params.id });
    if (!enquiry) {
      return res.status(404).json({ success: false, message: 'Business enquiry not found' });
    }
    res.json({ success: true, enquiry });
  } catch (error) {
    console.error('Fetch business enquiry error:', error);
    res.status(500).json({ success: false, message: 'Server error' });
  }
});

// PUT /api/business-enquiries/:id/status - Update business enquiry status
app.put('/api/business-enquiries/:id/status', async (req, res) => {
  try {
    const { status } = req.body;
    ...
    console.error('Update business enquiry status error:', error);
    res.status(500).json({ success: false, message: 'Server error' });
  }
});

// DELETE /api/business-enquiries/:id - Delete business enquiry
app.delete('/api/business-enquiries/:id', async (req, res) => {
  try {
    const enquiry = await BusinessEnquiry.findOneAndDelete({ id: req.params.id });
    if (!enquiry) {
      return res.status(404).json({ success: false, message: 'Business enquiry not found' });
    }
    res.json({ success: true, message: 'Business enquiry deleted successfully' });
  } catch (error) {
    console.error('Delete business enquiry error:', error);
    res.status(500).json({ success: false, message: 'Server error' });
  }
});
